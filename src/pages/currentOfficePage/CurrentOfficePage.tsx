...
const CurrentOfficePage = () => {
  ...
  const editPlant = (plant: IPlant, officeId: number) => {
    dispatch(plantSlice.actions.setPlant(plant))
    navigate(`${RouteNames.EDIT_PLANT}/${officeId}/${plant.id}`)
  ...
  const addPlant = () => {
    ...
    navigate(`${RouteNames.ADD_PLANT}/${officeId}`)
  ...
  if (!plants || plants?.length === 0) {
    return (
      <div className={styles.no_plants}>
        ...
        <Button classes={styles.no_plants_btn} onClick={addPlant}>
          ADD
        </Button>
      </div>
    )
          ...
          plants.map((plant, index) => (
            <li key={plant.id} className={[styles.plantListItem, index % 2 === 1 ? styles.plantListItem_grey : ''].join(' ')}>
              ...
              <div className={styles.edit_btn_wrapper}>
                <Button onClick={() => editPlant(plant, Number(officeId))}>Edit</Button>
                <Button classes={styles.redButton} onClick={() => deletePlant(plant.id)}>
                  Delete
                </Button>
              </div>
            </li>

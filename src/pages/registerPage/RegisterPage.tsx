...
const RegisterPage = () => {
    ...
    const password = useInput('admin')
    const confirmPassword = useInput('admin')
    ...
    return (
        <main className={styles.wrapper}>
            ...
            <Form classes={styles.form} onSubmit={registerUser}>
                ...
                <ErrorMessage classes={styles.error} isVisible={error !== ''}>{error}</ErrorMessage>
                <Select allData={offices} dataKey={'id'} dataValue={'address'} defaultValue={offices[0].id} onChange={onChangeSelect}/>
                <Button classes={styles.btn} disabled={isLoading}>REGISTER</Button>
                <Link className={styles.link} to={RouteNames.LOGIN}>Уже есть аккаунт? - Войдите!</Link>
            </Form>
        </main>
    );
};
